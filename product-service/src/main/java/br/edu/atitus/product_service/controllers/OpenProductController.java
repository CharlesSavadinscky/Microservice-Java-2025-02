...
import br.edu.atitus.product_service.repositoties.ProductRepository;
...
public class OpenProductController {
	...
	public ResponseEntity<ProductEntity> getProduct(@PathVariable Long idProduct, @PathVariable String targetCurrency)
		...
		String nameChache = "Product";
		String keyCache = idProduct + targetCurrency;

		ProductEntity product = cacheManager.getCache(nameChache).get(keyCache, ProductEntity.class);
		...
		if (product == null) {
			...
			if (product.getCurrency().equals(targetCurrency)) {
				product.setConvertedPrice(product.getPrince());

			} else {
				CurrencyResponse currency = currencyClient.getCurrency(serverPort, targetCurrency, targetCurrency);
				if (currency != null) {
					product.setConvertedPrice(currency.getConvertedValue());
					product.setEnviroment(product.getEnviroment() + " - " + currency.getEnviroment());
					
					cacheManager.getCache(nameChache).put(keyCache, product);
				} else {
	...
	public ResponseEntity<Page<ProductEntity>> getAllProducts(
	    ...
	    for (ProductEntity product : products) {
	        CurrencyResponse currency = currencyClient.getCurrency(
	                product.getConvertedPrice(), product.getCurrency(), targetCurrency);

	        product.setConvertedPrice(currency.getConvertedValue());

	        // Setar o ambiente
	        product.setEnvironment("Product-Service running on port: " + serverPort + " - " + currency.getEnviroment());
	    }
